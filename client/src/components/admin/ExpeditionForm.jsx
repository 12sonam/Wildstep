...
import { createExpedition, updateExpedition } from '../../services/api';

export default function ExpeditionForm({ expedition, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (expedition) {
        await updateExpedition(expedition._id, formData);
      } else {
        await createExpedition(formData);
      }
      onSuccess();
    } catch (err) {
      setError(err.response?.data?.message || 'Error saving expedition');
    } finally {
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="p-6 space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold text-gray-900">
          {expedition ? 'Edit Expedition' : 'Add New Expedition'}
        </h2>
        <button
          type="button"
          onClick={onClose}
